...
if mode == 1:
    ...
    first_path = '/home/kvasonaft/Development/spectra/dataset'
    names_table = '/home/kvasonaft/Development/spectra/names.csv'
    df = pd.read_csv(names_table, sep = ';')

    for _, row in df.iterrows():
        old_path = os.path.join(first_path, row['control_name'])
        new_path = os.path.join(first_path, str(row['new_name']))

        try:
            if os.path.exists(old_path):
                os.rename(old_path, new_path)
                print(f'Папка {row['control_name']} переименована в {row['new_name']}')
            else:
    ...
    second_path = '/home/kvasonaft/Development/spectra/dataset'
