...
def core(target = target, drop_culture=drop_culture, indicators_on_pictures=False, 
    ...
    for culture, data_1 in tqdm(data.items(), desc = 'Обработка культур'):
            ...
            for type, data_2 in data_1.items():
                ...
                try:
                    ...
                    if type == 'Control':
                        ...
                        indicator_peaks = round(np.nansum(np.array(dif_peaks)), 2)
                        indicator_area = round(np.nansum(np.array(dif_area)), 2)

                        dif_peaks = pd.Series(dif_peaks)
                        dif_area = pd.Series(dif_area)

                        if indicators_on_pictures == True:
                        ...
                        if indicators_on_pictures == False:
        ...
        plt.grid()
        plt.tight_layout()
        plt.savefig(f'graphs/spectra/{culture}.png', dpi = 300, bbox_inches = 'tight')
        plt.close()
