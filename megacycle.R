library(readr)
path <- "/home/kvasonaft/Documents/r_projects/Analysis of IR spectra/test/"
all_folders <- list.dirs(path, full.names = FALSE, recursive = FALSE)
folders_vector <- as.vector(all_folders)

for (item in folders_vector) {
  ...
  export_pca <- paste0("/home/kvasonaft/Documents/r_3projects/Analysis of IR spectra/pca_dataset/")
  ...
  difference_dataframe$damage[i] <- damage
  write_delim(difference_dataframe, paste0(export_pca, culture, " (", damage, ")", ".dpt"), delim = "\t")
